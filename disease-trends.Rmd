...
region_palette <- c(
  "New England"        = "#1b9e77",  # teal
  "Middle Atlantic"    = "#d95f02",  # orange
  "East North Central" = "#7570b3",  # purple
  "West North Central" = "#e7298a",  # magenta
  "South Atlantic"     = "#66a61e",  # green
  "East South Central" = "#e6ab02",  # golden
  "West South Central" = "#a6761d",  # brown-gold
  "Mountain"           = "#1f78b4",  # blue
  "Pacific"            = "#999999"   # grey (lower-burden coastal group)
)
...
plot_all_trends <- master_disease |>
  ...
  scale_color_manual(values = disease_palette, guide = "none") +
  scale_y_continuous(labels = comma) +
  labs(
    title    = "Reported Vector-Borne Disease Cases in the U.S. Over Time",
    subtitle = "Four separate panels make it easier to see each disease’s pattern on its own scale",
    x        = "Year",
...
<p style="margin-top:10px;">
Each panel shows the <strong>national case count over time</strong> for one disease, using a consistent color scheme across the website.
This avoids overlapping lines and makes each “heartbeat” easier to read.
</p>
...
plot_lyme_national <- lyme_year |>
  ...
  labs(
    ...
    subtitle = "Orange bars show yearly totals; the black curve highlights the long-term rise",
    x        = "Year",
    y        = "Reported Lyme cases"
  ) +
  theme(
    plot.title = element_text(color = disease_palette["Lyme"], face = "bold")
  )
...
```{r lyme-region-plot}
# Make sure factor levels line up with the palette (for consistent legend order)
lyme_region <- lyme_region |>
  mutate(region = factor(region, levels = names(region_palette)))
...
plot_lyme_region <- lyme_region |>
  ...
  theme(
    legend.position = "bottom",
    legend.title    = element_text(face = "plain")  # <- removes the bold look
  )
...
plot_west_nile <- west_nile |>
  ...
  geom_col(fill = disease_palette["West Nile"], alpha = 0.9) +
  scale_y_continuous(labels = comma) +
  labs(
...
plot_malaria <- malaria_cases |>
  ...
  geom_line(linewidth = 1.2, alpha = 0.9, color = disease_palette["Malaria"]) +
  geom_point(size = 1.8, alpha = 0.9, color = disease_palette["Malaria"]) +
  scale_y_continuous(labels = comma) +
  labs(
...
plot_dengue_week <- dengue_week |>
  ...
  geom_line(linewidth = 1.2, alpha = 0.9, color = disease_palette["Dengue"]) +
