...
# Malaria (national) – kept for consistency with other pages if needed later
...
cvi_summary |>
  ...
  row_spec(0, bold = TRUE, color = "white", background = accent_blue) |>
  column_spec(1, bold = TRUE)
```

```{r utah-cvi-rank-plot, fig.cap="Overall climate vulnerability scores for Utah compared with the most and least vulnerable states, with the U.S. median marked by a dashed line."}
# Build a simpler comparison: Utah + 5 most vulnerable + 5 least vulnerable
top_states <- cvi_state |>
  arrange(desc(`Overall CVI Score`)) |>
  slice_head(n = 5)

bottom_states <- cvi_state |>
...
ggplot(simplified_cvi_rank, aes(
  ...
  labs(
    title    = "Utah in Context: Overall Climate Vulnerability",
    subtitle = "Compared with the most and least vulnerable states; dashed line marks the U.S. median",
    x        = "Selected states",
    y        = "Overall CVI score"
  )
```
...
```{r lyme-plot, fig.cap="Lyme disease cases over time in Utah, the Mountain region, and the U.S. as a whole."}
...
# Summaries for all states
dengue_state_summary <- dengue_state |>
  group_by(location) |>
  summarise(total_cases = sum(cases, na.rm = TRUE), .groups = "drop")

# Build simplified comparison (Option B)
top_dengue <- dengue_state_summary |> arrange(desc(total_cases)) |> slice_head(n = 5)
bottom_dengue <- dengue_state_summary |> arrange(total_cases) |> slice_head(n = 5)
utah_only <- dengue_state_summary |> filter(location == "UT")

simp_dengue <- bind_rows(top_dengue, bottom_dengue, utah_only) |> distinct() |>
  mutate(is_utah = if_else(location == "UT", "Utah", "Other states"))
```
...
```{r dengue-simplified-plot, fig.cap="Dengue burden comparison showing Utah, the five highest-burden states, and the five lowest-burden states (2010–2024)."}
ggplot(simp_dengue, aes(
  ...
  scale_fill_manual(values = c("Utah" = utah_color, "Other states" = other_state_col)) +
  labs(
    title = "Dengue Cases (Selected States, 2010–2024)",
    subtitle = "Highest 5, lowest 5, and Utah highlighted",
    x = "State / territory",
...
```{r utah-lyme-climate-plot, fig.cap="Utah Lyme cases plotted alongside rescaled U.S. temperature anomalies over time."}
...
Created by Garrett Warr &middot; GEOG 3000 – Climate Change in Science &amp; Society &middot; Data and analyses for educational purposes only.
</p>
